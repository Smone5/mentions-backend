...
async def get_current_user(
    ...
    try:
        ...
        # Create client with anon key but set the user's token
        ...
        user_profile = UserProfile(
            ...
            email=user_email,  # Use email from auth response, not from profile table
    ...
    async def role_checker(current_user: UserProfile = Depends(get_current_user)) -> UserProfile:
        ...
        if not current_user.role or current_user.role not in allowed_roles:
            logger.warning(
                ...
                f"attempted to access endpoint requiring: {', '.join(allowed_roles)}"
            )
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
